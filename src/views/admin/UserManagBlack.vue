<template>
  ...
  >
      ...
      <template #default="scope">
        <div>
          ...
          <span v-else-if="column.prop === 'touxiang'">
            <template v-if="infoValidateForm.touxiang">
              <img
    ...
    <el-table-column label="操作" min-width="80">
      <template #default="scope">
        <el-popconfirm title="确定要解除该用户黑名单?">
          <template #reference>
            <el-button link type="success" size="small"> 解除黑名单 </el-button>
          </template>
...
const columns = ref([
  ...
  { prop: "yonghuming", label: "用户名", width: "150" },
...
const fetchData = async () => {
  try {
    ...
    const { list, totalPage, currPage } = await fetchUserBlackPage(
      query,
...
//add 黑名单
const delBlacklist = async (row) => {
  console.log(row);
  const msg = await fetchUserDelBlacklist({ ids: row.id });
  if (msg == 0) {
    ElMessage({
      type: "success",
      message: "黑名单解除成功",
