...
<script setup>
import { fetchFavorListPage, fetchFavorDel } from "@/services/userServices";
...
const fetchData = async () => {
  try {
    ...
    const { list, totalPage, currPage } = await fetchFavorListPage(
      query,
  ...
  if (selectedRows.value.length <= 0) {
    ...
    ElMessageBox.confirm("确认删除选中行?", "删除", {
      ...
      .then(async () => {
        ...
        console.log(ids);
        const msg = await fetchFavorDel(ids);
        if (msg == 0) {
          ElMessage({
            type: "success",
            message: "删除成功",
...
const delConfirm = async (row) => {
  console.log(row);
  const msg = await fetchFavorDel([row.id]);
  if (msg == 0) {
    ElMessage({
      type: "success",
      message: "删除成功",
